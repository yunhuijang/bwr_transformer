...
class Sampler(object):
    ...
    def sample(self, save_name=None):
        # -------- Load checkpoint --------
        self.ckpt_dict = load_ckpt(self.config, self.device, save_name)
...
class Sampler_mol(object):
    ...
    def sample(self, save_name=None):
        ...
        with open((f'output/GDSS_{self.config.data.data}_list.txt'), 'w') as f:
            for smiles in gen_smiles:
                f.write(f'{smiles}\n')

        # -------- Evaluation --------
        scores = get_all_metrics(gen=gen_smiles, device=self.device[0], n_jobs=8, test=test_smiles, train=train_smiles)
        ...
        #WANDB
        wandb.init(name=f'{self.config.data.data}-GDSS', project="stgg-src")
