...
def load_ckpt(config, device, save_name=None, ts=None, return_ckpt=False):
    device_id = f'cuda:{device[0]}' if isinstance(device, list) else device
    ckpt_dict = {}
    if ts is not None:
        config.ckpt = ts
    path = f'./checkpoints/{config.data.data}/{config.ckpt}_.pth'
    if save_name is not None:
        path = f'./checkpoints/{config.data.data}/{config.ckpt + save_name}.pth'
