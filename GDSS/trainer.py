...
class Trainer(object):
    ...
    def train(self, ts):
        ...
        self.ckpt = f'{ts}'
        self.config.ckpt = f'{ts}'
        # if self.config.data.data == "QM9":
        #     self.config.ckpt = f'gdss_qm9'
        # elif self.config.data.data == "ZINC250k":
        #     self.config.ckpt = f'gdss_zinc250k'
        # else:
        #     self.config.ckpt = None
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            if epoch % self.config.train.check_sample_every_n_epoch == self.config.train.check_sample_every_n_epoch-1 and epoch > 0:
                if self.config.data.data in ['QM9', 'ZINC250k']:
                    sampler = Sampler_mol(self.config)
                else:
                    sampler = Sampler(self.config) 
                sampler.sample(save_name)
