...
def main(config, print_metrics=True):
    ...
    test_graphs = mols_to_nx(smiles_to_mols(test_scaffolds))
    gen_graphs = mols_to_nx(smiles_to_mols(gen))
    nspdk_score = compute_nspdk_mmd(test_graphs, gen_graphs, metric='nspdk',
                                    is_hist=False)
    metrics['NSPDK'] = nspdk_score  
    wandb.log(metrics)
